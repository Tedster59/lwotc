...
function XComGameState InterventionAbility_BuildGameState( XComGameStateContext Context )
{
	...
	if (UiTimer != none)
	{
		...
		AdjustKismetMissionTimerVariable(default.INTERVENTION_EXTRA_TURNS);
	}
...
//method that bypasses the Fxs KismetVariable Manager - from LW Officer notes
private function AdjustKismetMissionTimerVariable(int Adjustment)
{
	local X2SitRepEffect_ModifyKismetVariable SitRep;
	local XComGameState 		NewGameState;

	//PrintKismetVariables(default.KismetTimerVariableNames);
	class'Helpers'.static.OutputMsg("Adjustment Required: " $ Adjustment);
	...
	class'Helpers'.static.OutputMsg("Adjustment Submitted");

	//PrintKismetVariables(default.KismetTimerVariableNames);
	class'Helpers'.static.OutputMsg("!! NOTE !! - IF THE ABOVE VALUES ARE NOT CORRECT");
	class'Helpers'.static.OutputMsg("-- -- ALLOW ONE TURN TO PASS AND TRY AGAIN -- --");
}
		...
		if(SeqVar != none)
		{
			...
			if(SeqVarTimer != none)
			{
				...
				if(VariableNames.Find(string(SeqVarTimer.VarName)) != INDEX_NONE)
				{
					...
					//SeqVarTimer.IntValue = SeqVarTimer.IntValue + Adjustment;
					`LOG("Named KismetVariable: " $ SeqVar.VarName $ ", Value= " $ SeqVarTimer.IntValue, default.bEnableLogging,'RulersPauseTimers');
